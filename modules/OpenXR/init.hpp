...
XrApplicationInfo createXrApplicationInfo(XrVersion apiVersion, const char* applicationName, uint32_t applicationVersion, const char* engineName, uint32_t engineVersion)
{
    XrApplicationInfo res;
    res.apiVersion = apiVersion;
    strcpy(res.applicationName, applicationName);
    res.applicationVersion = applicationVersion;
    strcpy(res.engineName, engineName);
...
void init_openxr()
{
    ...
    uint32_t out_l;
    xrEnumerateInstanceExtensionProperties(nullptr, 0, &out_l, nullptr);
    std::vector<XrExtensionProperties> props(out_l, {XR_TYPE_EXTENSION_PROPERTIES});
    xrEnumerateInstanceExtensionProperties(nullptr, out_l, &out_l, props.data());

    std::cout << ">> Got " << out_l << " extensions" << std::endl;

    info.applicationInfo = createXrApplicationInfo(XR_CURRENT_API_VERSION, "FullBowody", 1, "", 1);
    info.enabledExtensionCount = 1;
    const char* a[1] = {"XR_EXT_hand_tracking"};
    info.enabledExtensionNames = a;

    std::cout << "-> xrCreateInstance(info, instance);" << std::endl;
...
{
    std::cout << "-> xrDestroyInstance(instance);" << std::endl;
